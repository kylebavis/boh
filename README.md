...
- Several source URLs per post, added and removed by hand on the post, and searchable with `url:pixiv.net`. An import records the page each file came from where the site reports one, falling back to the URL you typed
...
```
url:pixiv.net                         posts sourced from pixiv
url:donmai.us -url:pixiv.net          on danbooru but not pixiv
landscape url:pixiv.net               combines with tags like any other term
...
The prefix is `url:` rather than the `source:` other boorus use because `source` is already a tag namespace here: an import stores the site it came from as a tag like `source:danbooru`, and `source:` in a search still finds those tags. The one address you cannot search for is the literal word `none`.
